...
// return True if the current node is the tail node or
// if the list and the current position are both empty.
...
// to update the data portion of the current node to contain (d);
// assume the current position is nonempty.
template <class keyType, class dataType>
void List<keyType, dataType>::updateData(const dataType &d)
{	cursor -> data = d; }

// to return the key and data in the current node; assume the current
// position is nonempty.
template <class keyType, class dataType>
void List<keyType, dataType>::retrieve (keyType &k, dataType &d) const
{	
	k = cursor-> key;  d = cursor->data; 
}
// insert a node with key (k) and data (d) at the head of the list; 
...
// insert a node with key (k) and data (d) after the current node
...
// insert a node with key (k) and data (d) before the current
// node, current position becomes the new node.
template <class keyType, class dataType>
...
// insert a node with key (k) and data (d) at the end
// of the list, current position becomes the new node.
template <class keyType, class dataType>
...
// if the current node is the last node initially, the
...
void List<keyType, dataType>::deleteNode()
{
	...
	if (atFirst()) 	// delete head node
   		{	q = cursor; 	cursor = cursor->next;	
			head = cursor;  delete q;   
		}

else 		// delete non-head node
   		{	q = cursor;   cursor = cursor->next;
			prev->next = cursor;	delete q;    
		}
   }
}	
...
/* search the list for the node with key part that matches (k). If found, set cursor to the node and return True,else return false and the current position becomes empty. */
template <class keyType, class dataType>
...
// insert a node with key (k) and data (d) in a position that
// maintains an ascending order of the key portion of the nodes.
template <class keyType, class dataType>
...
template <class keyType, class dataType>
bool List <keyType, dataType> :: sameAs( List <keyType, dataType> &L2){
	int x1 = listSize(); //size of first list
	int x2 = L2.listSize(); //size of second list
	node *ptr1 = head; //pointer to the head of first list
	node *ptr2 = L2.head; //pointer to the head of second list
	if(x1 != x2) //if the sizes are not the same, then automatically they are not the same
		return false;
	while(ptr1 != NULL){ //while not the end of the list
		if(ptr1->data != ptr2->data) //compare the data of the two lists (if not equal then return false, otherwise continue looping)
			return false;
		ptr1 = ptr1->next; //move pointer of first list
		ptr2 = ptr2->next; //move pointer of second list
	}
	return true; // return true if sizes are equal and the values are the same
	}
